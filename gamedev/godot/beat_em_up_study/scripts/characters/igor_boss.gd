...
func _process(delta: float) -> void:
	...
	hit_box.monitoring = is_attacking()
...
func handle_grounded() -> void:
	...
	elif state == State.RECOVERY and Time.get_ticks_msec() - time_start_vulnerable > duration_vulnerable:
		state = State.IDLE
...
func on_receive_damage(amount: int, direction: Vector2, _hit_type: HurtBox.HitType) -> void:
...
func on_emit_damage(target_hurt_box: HurtBox) -> void:
	target_hurt_box.damage_received.emit(damage, heading, HurtBox.HitType.KNOCKDOWN)
	time_last_attack = Time.get_ticks_msec()
	state = State.IDLE
