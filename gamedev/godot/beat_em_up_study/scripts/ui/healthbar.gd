...
func refresh(current_health: int, max_health: int) -> void:
	var reverse = -1 if is_inverted else 1
	white_border.scale.x = (max_health + 2) * reverse
	content_background.scale.x = max_health * reverse
	health_gauge.scale.x = current_health * reverse
